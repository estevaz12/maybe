...
- See `.ruby-version` for the required Ruby version (if you use [mise](https://mise.jdx.dev/), run `mise trust` in the repo root once so `mise.toml` applies, then `mise install`). Other managers (rbenv, asdf) should install the same version.
- PostgreSQL — recent stable release (CI uses the official Postgres image).
- Redis — required for the Sidekiq worker started by `bin/dev` (default `redis://127.0.0.1:6379/1` unless you override it in `.env.local`).
- Node.js — required for JavaScript lint/format (`npm install` is run by `bin/setup` when `package.json` is present). CI uses Node 20.
...
The app exposes a Rails health check at `GET /up` (useful for load balancers and smoke checks). For AI agents and automation, see [AGENTS.md](AGENTS.md).
